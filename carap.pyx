...
def compute_mask(mask, orig, width, height, tolerance):
    """
...
def points(left, right, swap, x0, y0, x1, y1):
    if swap:
        x0, y0 = y0, x0
        x1, y1 = y1, x1
    dx = abs(x1 - x0)
...
def rasterize(corners, left, right):
    for i in range(4):
        x0 = corners[i][0]
        y0 = corners[i][1]
        x1 = corners[(i + 1) % 4][0]
...
def project(homography, mask, orig, data,
